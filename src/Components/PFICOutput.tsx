                        ...
                        <tr><td colSpan={2}>
                            <table className="ShareBlocks">
                                <tr className="firstHeaerRow">
                                    ...
                                    <td className="columnLastCell" colSpan={pfic.ShareHoldingYears.size}>Number of Holding Days</td>
                                    <td className="bottomBorder columnLastCell" rowSpan={2}>16B</td>
                                    <td colSpan={pfic.ShareHoldingYears.size - 1}>16C</td>
                                </tr>
                                ...
                                </tr>
                                {
                                    pfic.ShareBlocks.map((shareBlock: ShareBlock, index: number) =>
                                        <tr>
                                            ...
                                            <td>{shareBlock.PurchaseDate.toString()}</td>
                                            <td className="columnLastCell">{shareBlock.PurchaseAmount}</td>
                                            <td>{shareBlock.DisposeDate.toString()}</td>
                                            <td className="columnLastCell">{shareBlock.DisposeAmount}</td>
                                ...
                                <tr className="totalRow"><td>Total</td><td className="columnLastCell"></td>
                                    <td ></td>
                                    <td className="columnLastCell">{pfic.TotalPurchaseAmount}</td>
                                    <td></td>
                                    <td className="columnLastCell">{pfic.TotalDisposeAmount}</td>
                                    <td className="columnLastCell">{pfic.TotalGain}</td>
                                    <td className="columnLastCell" colSpan={pfic.ShareHoldingYears.size}></td>
                                    <td className="columnLastCell">{pfic.TotalOtherIncome}</td>
                                    {
                                        Array.from(pfic.ShareHoldingYears.values()).filter(a => a.Year != 2022).map((shareHoldingYear: ShareHoldingYear, index: number) =>

                                            <td className="rightAlignCell">{shareHoldingYear.TotalGain}</td>
                                        )
                                    }
                                </tr>
                            </table>
